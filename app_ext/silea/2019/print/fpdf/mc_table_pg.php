...
function SetBorderColor($co)
{
	$this->colors=$co;
}

function SetFontStyle($fs)
{
    //Set the array of column widths
    $this->fontstyles=$fs;
}
...
function Row($data)
{
    ...
    for($i=0;$i<count($data);$i++)
    {
        ...
        $fs=isset($this->fontstyles[$i]) ? $this->fontstyles[$i] : '';
        $co=isset($this->colors[$i]) ? $this->colors[$i] : '255';
	...
	$this->SetDrawColor($co,$co,$co);
        $this->Rect($x,$y,$w,$h);
        //Print the text

        $this->SetFont("Arial", $fs, 10);
