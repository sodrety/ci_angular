...
function app_baca_limit($tb,$wh,$fd,$l) {
$db = new database();
$db->connect(app_db());
$query = "SELECT $fd FROM $tb WHERE $wh LIMIT ".$l.",1";
$results = $db->get_results( $query );
foreach( $results as $row) {
return $row[$fd];
}
...
function app_jml($tb,$wh) {
$db = new database();
$db->connect(app_db());
$query = "SELECT id FROM $tb WHERE $wh";
$results = $db->get_results( $query );
$n=0;
foreach( $results as $row) {
$n+=1;
