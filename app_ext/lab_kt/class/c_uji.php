...
function uji_lot_print($id) {
$db = new database();
$db->connect(app_db());
$query = "SELECT * from uji_lot where id_uji='".$id."' order by lot_kode";
$results = $db->get_results( $query );
foreach( $results as $data )
{
$n++;
$dt.= $n.". ".$data['lot_kode']." ".$data['nama']." (".$data['ket'].") 
";
}
if ($n=="") {$dt="Terlampir";}
return $dt;
...
if ($data['distribusi_tgl']=="") {$bg=" bgcolor='#dddddd'";$distribusi="";} else {$bg="";$distribusi="<a target='_blank' href='print/distribusi.php?&id=".$data['id']."'>Print</a> ";}
...
{
$n++;
if ($n==1) {$print=$distribusi."<a href='?t=FormDistribusi&k=".$data['kode']."'>Edit</a>";$g="";} else {$print="";$g=" class='gray'";}

echo "<tr><td".$g.">".$no."</td><td".$g.">".$data['distribusi_tgl']."</td><td".$g.">".$data['kode']."</td><td".$g.">".$data['komoditi']."</td><td".$g.">".$data['distribusi_kondisi']."</td><td".$g."  align=center>".app_jml("uji_lot","id_uji='".$data['id']."'","id")."</td><td".$g.">".$data['terima_tgl']."</td><td>".$n."</td><td>".$data0['target_kode']."</td><td>".$data0['nama_latin']."</td><td>".$data0['jenis']."</td><td>".app_baca("m_metode","id='".$data0['id_metode']."'","metode")."</td><td></td><td>".$data0['ket']."</td><td>".$print."</td></tr>";
}
...
$db->connect(app_db());
if ($_POST) {
$query = "SELECT idt from uji_target WHERE id_uji='".$id."'";
$results = $db->get_results( $query );
foreach( $results as $data )
{
$m="m".$data['idt'];
$k="k".$data['idt'];
app_update("uji_target","idt='".$data['idt']."'","id_metode='".$_POST[$m]."',ket='".$_POST[$k]."'");
}
}

$query = "SELECT * from uji_target join m_optk ON uji_target.id_target=m_optk.id where id_uji='".$id."' order by jenis,nama_latin";
$results = $db->get_results( $query );
$dt.= "<table class=table><tr><th>No</th><th>Kode Target</th><th>Nama OPTK</th><th>Jenis</th><th>Metode</th><th>Keterangan</th></tr>";

foreach( $results as $data )
{
$n++;
$dt.= "<tr><td>".$n."</td><td>".$data['target_kode']."</td><td>".$data['nama_latin']."</td><td>".$data['jenis']."</td><td><select name='m".$data['idt']."' id=select".$n.">".$opsi->m_metode($data['id_metode'])."</select></td><td><input type=text name='k".$data['idt']."' value='".$data['ket']."' size='60'></td></tr>";
}
...
elseif ($m=="5") {
	$img=app_baca("uji_img","idh='".$idh."'","img");
$td="<form method='post' target='iframe' action='exe/uji_hasil.php' enctype='multipart/form-data'>
<td>
<a href='img/".$img."' target='_blank'>".$img."</a>
<input type='file' name='file' onchange=\"javascript:this.form.submit();\"></td>"; }
...
$selesai_tgl=app_baca("uji_hasil","id='".$idh."'","selesai_tgl");

if ($selesai_tgl=="") {$selesai_tgl=today();}

echo "

<tr><td>".$nn."</td><td>".$kode."1".$data['lot_kode']."</td><td>".$data['nama']."</td>".$td."
<td>
<input type=hidden name=id_uji value='".$id."'>
<input type=hidden name=idh value='".$idh."'>
<input type=hidden name='idt' value='".$idt."'><input type=hidden name='lot_kode' value='".$data['lot_kode']."'>
<input type=date name='selesai_tgl' value='".$selesai_tgl."' onchange=\"javascript:this.form.submit();\"></td><td><select name=hasil onchange=\"javascript:this.form.submit();\">".$opsi->h_uji($idh)."</select></td><input type=hidden name=id value='".$idh."'><input type=hidden name=id_uji value='".$id."'></tr></form>";
...
function uji_analis($id) {
$db = new database();
$db->connect(app_db());
$query = "SELECT * from uji_analis where id_uji='".$id."' order by id_analis";
$results = $db->get_results( $query );
$kode=app_baca("uji","id='".$id."'","kode");	
foreach( $results as $data )
{
$n++;
$dt.= "<tr><td>".$n."</td><td>".db_baca("user_login","karyawan","id='".$data['analis']."'","nama")."</td><td><a href='exe/uji_analis_del.php?id_analis=".$data['id_analis']."' target='iframe' onClick=\"javascript:requestContent('ajx/uji_analis.php?id=".$id."','uji_analis')\"> x </a></td></tr>";
}
